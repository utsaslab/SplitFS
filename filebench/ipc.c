...
void ipc_init(void)
{
	...
	printf("%s: performing memset, size = %lu\n", __func__, (char *)&filebench_shm->shm_marker - (char *)filebench_shm);
	(void) memset(filebench_shm, 0, (char *)&filebench_shm->shm_marker - (char *)filebench_shm);
	printf("%s: memset done\n", __func__);
