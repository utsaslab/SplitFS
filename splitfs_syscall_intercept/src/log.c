...
static uint8_t clearing_app_log;
static uint8_t clearing_op_log;
static loff_t app_log_tail;
static loff_t op_log_tail;
static loff_t app_log_lim;
static loff_t op_log_lim;
static int app_log_fd;
static int op_log_fd;
static unsigned long app_log;
static unsigned long op_log;
static struct inode_path *ino_path_head;
...
void init_logs() {
	...
	app_log_fd = syscall_no_intercept(SYS_open, APPEND_LOG_PATH, O_RDWR | O_CREAT, 0666);		
	if (app_log_fd < 0) {
		MSG("%s: Creation of append log file failed. Err = %s\n",
		    __func__, strerror(-app_log_fd));
		assert(0);
	}
	op_log_fd = syscall_no_intercept(SYS_open, OP_LOG_PATH, O_RDWR | O_CREAT, 0666);		
	if (op_log_fd < 0) {
		MSG("%s: Creation of op log file failed. Err = %s\n",
		    __func__, strerror(-op_log_fd));
		assert(0);
	}
...
static void sync_and_clear_app_log() {
	int i = 0, ret = -1;
...
static void sync_and_clear_op_log() {
	int i = 0, ret = -1;
	struct NVFile *nvf = NULL;
	int cpuid = GET_CPUID();
