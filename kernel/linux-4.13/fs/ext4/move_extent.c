		     ...
		     __u64 donor_start, __u64 *len)
{
	...
	if ((!orig_inode->i_size) || (!donor_inode->i_size)) {
		...
		//printk(KERN_INFO "%s: File size is 0 byte. orig_inode->i_size = %lu, donor_inode->i_size = %lu\n", __func__, orig_inode->i_size, donor_inode->i_size);
		return -EINVAL;
	}
	...
	if ((orig_start & ~(PAGE_MASK >> orig_inode->i_blkbits)) !=
		...
		ext4_debug("ext4 move extent: orig and donor's start "
			...
			orig_inode->i_ino, donor_inode->i_ino);
		printk(KERN_INFO "%s: offsets are not same. orig_start = %lld, donor_start = %lld\n", __func__, orig_start, donor_start);
		return -EINVAL;
	}
	...
	if ((orig_start >= EXT_MAX_BLOCKS) ||
		...
		ext4_debug("ext4 move extent: Can't handle over [%u] blocks "
			...
			orig_inode->i_ino, donor_inode->i_ino);
		printk(KERN_INFO "%s: can't handle too many blocks.\n", __func__);
		return -EINVAL;
	}
	...
	if (!*len) {
		ext4_debug("ext4 move extent: len should not be 0 "
			"[ino:orig %lu, donor %lu]\n", orig_inode->i_ino,
			donor_inode->i_ino);
		printk(KERN_INFO "%s: len should not be 0.\n", __func__);
		return -EINVAL;
	}
